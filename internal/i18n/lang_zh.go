...
install 选项:
  ...
  --repo <URL>      预设配置的 Git 仓库地址 (可选, 如 https://github.com/user/repo)`,
	...
	"create.subvol.failed":   "创建 Btrfs 子卷失败: %s: %v",
  ...
  └── logs/            # 构建日志`,
	...
	"chroot.paru.create.user":        "[Chroot] 创建 builder 用户...",
	"chroot.paru.create.user.failed": "[Chroot] 创建 builder 用户失败: %v",
	...
	"cfg.no.layers":          "config.yaml 中未定义任何层",
	"cfg.layer.not.exist":    "config.yaml 引用的层文件不存在: %s",
	"cfg.layer.unreferenced": "layers/ 目录中存在未被 config.yaml 引用的文件: %s",
